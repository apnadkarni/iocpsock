 ...
 * win32err.c --
 *
 *	Converts Win32 errors into a form friendly for Tcl.  This makes up
 *	for the brain-dead POSIX translations.
 *
 * ----------------------------------------------------------------------------
 * RCS: @(#) $Id$
 * ----------------------------------------------------------------------------
...
CONST char *
Iocp_Win32ErrId (DWORD errorCode)
{
    switch (errorCode) {
 ...
 * IocpExpErrId --
 *
 *	Same as Tcl_ErrnoId(), but for Iocpsock specific error codes.
 *
 * Results:
 *      The symbolic name of the error code.
 *    
 * Comments:
 *	Must match iocpErr.h which is a build generated file!
 *
 * Side Effects:
 *	None.
 *
...
CONST char *
IocpErrId (DWORD errorCode)
{
    switch (errorCode) {
	case ERROR_IOCP_WSE_NOTFOUND :		return "ERROR_IOCP_WSE_NOTFOUND";
	case ERROR_IOCP_WSE_CANTDOONEPOINTZERO:	return "ERROR_IOCP_WSE_CANTDOONEPOINTZERO";
	case ERROR_IOCP_WSE_NOTALL11FUNCS :	return "ERROR_IOCP_WSE_NOTALL11FUNCS";
	case ERROR_IOCP_WSE_NOTALL2XFUNCS :	return "ERROR_IOCP_WSE_NOTALL2XFUNCS";
	case ERROR_IOCP_WSE_CANTSTARTCMPTHREAD:	return "ERROR_IOCP_WSE_CANTSTARTCMPTHREAD";
	DEFAULT_UNREACHABLE;
    }
}
 ...
 * IocpErrMsg --
 *
 *	Same as Tcl_ErrnoMsg(), but for Iocpsock specific error codes.
 *
 * Results:
 *      The message that is associated with the error code.  Not
 *	localized.
...
CONST char *
IocpErrMsg TCL_VARARGS_DEF(DWORD, arg1)
{
    va_list argList;
    DWORD errorCode;
    CONST char *msg;
    ...
    errorCode = TCL_VARARGS_START(DWORD, arg1, argList);
    msg = IocpErrMsgVA(errorCode, argList);
    va_end(argList);
    return msg;
}
...
CONST char *
IocpErrMsgVA (DWORD errorCode, va_list argList)
{
    ThreadSpecificData *tsdPtr = TCL_TSD_INIT(&dataKey);
    DWORD result;
...
CONST char *
IocpError TCL_VARARGS_DEF(Tcl_Interp *, arg1)
{
    CONST char *id, *msg;
    va_list argList;
    Tcl_Interp *interp;
    ...
    if (err & (0x1 << 29)) {
	id = IocpErrId(err);
	msg = IocpErrMsgVA(err, argList);
	Tcl_SetErrorCode(interp, "IOCPSOCK", id, msg, 0L);
    } else {
	id = Tcl_Win32ErrId(err);
	msg = Tcl_Win32ErrMsgVA(err, argList);
	Tcl_SetErrorCode(interp, "WIN32", id, msg, 0L);
